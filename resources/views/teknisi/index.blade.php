    ...
    <main class="flex-1 p-gutter overflow-y-auto" x-data="{
            ...
            <div class="flex items-end justify-between">
                <div class="flex flex-col gap-xs">
                    <div class="flex items-center gap-xs font-body-sm text-body-sm text-on-surface-variant mb-2">
                        ...
                        <span class="font-medium text-on-surface">Teknisi</span>
                    </div>
                ...
                <table id="teknisiTable" class="w-full text-left border-collapse min-w-[800px] stripe hover" style="width:100%">
                    <thead>
                        <tr class="bg-surface-container-low border-b border-surface-variant">
                            ...
                            <th class="font-label-caps text-label-caps text-on-surface-variant py-sm px-md font-semibold">Nama Teknisi</th>
                    ...
                    <tbody class="font-data-tabular text-data-tabular text-on-surface">
                        @foreach($teknisis as $index => $t)
                        <tr class="border-b border-surface-variant transition-colors">
                            <td class="py-sm px-md font-medium">{{ $index + 1 }}</td>
                            <td class="py-sm px-md">
                                <div class="flex items-center gap-xs">
                                    <div class="w-8 h-8 rounded-full bg-tertiary-fixed text-on-tertiary-fixed flex items-center justify-center font-bold text-xs uppercase shrink-0">
                                        {{ substr(preg_replace('/[^a-zA-Z]/', '', $t->nama), 0, 2) }}
                                    </div>
                                    {{ $t->nama }}
                                </div>
                            </td>
                            ...
                            <td class="py-sm px-md text-right">
                                <button @click="openEditModal({{ json_encode($t) }})" class="text-outline hover:text-secondary p-1 rounded hover:bg-surface-variant transition-colors"><span class="material-symbols-outlined text-[20px]">edit</span></button>
                                <button @click="openDeleteModal({{ json_encode($t) }})" class="text-outline hover:text-error p-1 rounded hover:bg-surface-variant transition-colors"><span class="material-symbols-outlined text-[20px]">delete</span></button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
        ...
        <div x-show="showAddModal" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4" style="display: none;">
            <div @click.away="showAddModal = false" x-transition class="bg-surface-container-lowest rounded-xl shadow-xl w-full max-w-sm overflow-hidden">
                <div class="px-4 py-3 border-b border-surface-variant flex justify-between items-center bg-surface">
                    <h3 class="font-h3 text-h3 text-on-surface">Tambah Teknisi</h3>
                ...
                <form action="{{ route('teknisi.store') }}" method="POST">
                    ...
                    <div class="p-4 flex flex-col gap-3">
                        <div>
                            <label class="block text-sm font-medium text-on-surface mb-1">Nama Teknisi</label>
        ...
        <div x-show="showEditModal" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4" style="display: none;">
            <div @click.away="showEditModal = false" x-transition class="bg-surface-container-lowest rounded-xl shadow-xl w-full max-w-sm overflow-hidden">
                <div class="px-4 py-3 border-b border-surface-variant flex justify-between items-center bg-surface">
                    <h3 class="font-h3 text-h3 text-on-surface">Edit Teknisi</h3>
                ...
                <form :action="`/teknisi/${selectedTeknisi?.id}`" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="p-4 flex flex-col gap-3">
                        <div>
                            <label class="block text-sm font-medium text-on-surface mb-1">Nama Teknisi</label>
                            <input type="text" name="nama" x-model="selectedTeknisi && selectedTeknisi.nama" required class="w-full bg-surface-container-lowest border border-outline-variant rounded-md px-3 py-2 text-on-surface focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all">
                        </div>
                        <div class="flex items-center gap-2 mt-2">
                            <input type="checkbox" name="is_active" id="is_active_edit" value="1" :checked="selectedTeknisi?.is_active" class="w-4 h-4 rounded border-outline-variant text-secondary focus:ring-secondary">
        ...
        <div x-show="showDeleteModal" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4" style="display: none;">
            <div @click.away="showDeleteModal = false" x-transition class="bg-surface-container-lowest rounded-xl shadow-xl w-full max-w-sm overflow-hidden">
                <div class="p-6 text-center">
                    ...
                    <h3 class="font-h3 text-h3 text-on-surface mb-2">Hapus Teknisi?</h3>
                    <p class="text-sm text-on-surface-variant mb-6">Apakah Anda yakin ingin menghapus teknisi <strong x-text="selectedTeknisi?.nama"></strong>? Tindakan ini tidak dapat dibatalkan.</p>
                    
                    <form :action="`/teknisi/${selectedTeknisi?.id}`" method="POST" class="flex gap-2 justify-center">
                        @csrf
                        @method('DELETE')
