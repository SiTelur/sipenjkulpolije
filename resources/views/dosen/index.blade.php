    ...
    <main class="flex-1 p-gutter overflow-y-auto" x-data="{
        ...
        <div class="max-w-[1440px] mx-auto flex flex-col gap-lg">
            ...
            <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-3">
                <div class="flex flex-col gap-xs">
                    <div class="flex items-center gap-xs font-body-sm text-body-sm text-on-surface-variant mb-2">
                        ...
                        <span class="font-medium text-on-surface">Dosen</span>
                    </div>
                    <h1 class="font-h1 text-h1 text-on-surface">Manajemen Dosen</h1>
                </div>
            ...
            <div class="bg-surface-container-lowest border border-outline-variant rounded-lg shadow-[0_4px_20px_rgba(15,23,42,0.04)] overflow-hidden p-4">
                ...
                <table id="dosenTable" class="w-full text-left border-collapse min-w-[600px] stripe hover" style="width:100%">
                    <thead>
                        <tr class="bg-surface-container-low border-b border-surface-variant">
                            <th class="font-label-caps text-label-caps text-on-surface-variant py-sm px-md font-semibold">NIDN</th>
                            <th class="font-label-caps text-label-caps text-on-surface-variant py-sm px-md font-semibold">Nama Lengkap</th>
                            <th class="font-label-caps text-label-caps text-on-surface-variant py-sm px-md font-semibold">Tipe Dosen</th>
                            <th class="font-label-caps text-label-caps text-on-surface-variant py-sm px-md font-semibold">Status Aktif</th>
                    ...
                    <tbody class="font-data-tabular text-data-tabular text-on-surface">
                        ...
                        <tr class="border-b border-surface-variant transition-colors">
                            <td class="py-sm px-md font-medium">{{ $d->nidn }}</td>
                            <td class="py-sm px-md">
                                <div class="flex items-center gap-xs">
                                    <div class="w-8 h-8 rounded-full bg-primary-fixed text-on-primary-fixed flex items-center justify-center font-bold text-xs uppercase shrink-0">
                                        {{ substr(preg_replace('/[^a-zA-Z]/', '', $d->nama), 0, 2) }}
                                    </div>
                                    {{ $d->nama }}
                                </div>
                            </td>
                            <td class="py-sm px-md text-on-surface-variant">{{ str_replace('_', ' ', $d->tipe_dosen->value ?? $d->tipe_dosen) }}</td>
                            <td class="py-sm px-md">
                                @if($d->is_active)
                            ...
                            <td class="py-sm px-md text-right">
                                <button @click="openEditModal({{ json_encode($d) }})" class="text-outline hover:text-secondary p-1 rounded hover:bg-surface-variant transition-colors"><span class="material-symbols-outlined text-[20px]">edit</span></button>
                                <button @click="openDeleteModal({{ json_encode($d) }})" class="text-outline hover:text-error p-1 rounded hover:bg-surface-variant transition-colors"><span class="material-symbols-outlined text-[20px]">delete</span></button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
        ...
        <div x-show="showAddModal" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4" style="display: none;">
            <div @click.away="showAddModal = false" x-transition class="bg-surface-container-lowest rounded-xl shadow-xl w-full max-w-sm overflow-hidden">
                <div class="px-4 py-3 border-b border-surface-variant flex justify-between items-center bg-surface">
                    <h3 class="font-h3 text-h3 text-on-surface">Tambah Dosen</h3>
                ...
                <form action="{{ route('dosen.store') }}" method="POST">
                    ...
                    <div class="p-4 flex flex-col gap-3">
                        <div>
                            <label class="block text-sm font-medium text-on-surface mb-1">NIDN</label>
                            <input type="text" name="nidn" required class="w-full bg-surface-container-lowest border border-outline-variant rounded-md px-3 py-2 text-on-surface focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all">
                        </div>
                        <div>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-on-surface mb-1">Tipe Dosen</label>
                            <select name="tipe_dosen" required class="w-full bg-surface-container-lowest border border-outline-variant rounded-md px-3 py-2 text-on-surface focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all">
        ...
        <div x-show="showEditModal" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4" style="display: none;">
            <div @click.away="showEditModal = false" x-transition class="bg-surface-container-lowest rounded-xl shadow-xl w-full max-w-sm overflow-hidden">
                <div class="px-4 py-3 border-b border-surface-variant flex justify-between items-center bg-surface">
                    <h3 class="font-h3 text-h3 text-on-surface">Edit Dosen</h3>
                ...
                <form :action="`/dosen/${selectedDosen?.id}`" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="p-4 flex flex-col gap-3">
                        <div>
                            <label class="block text-sm font-medium text-on-surface mb-1">NIDN</label>
                            <input type="text" name="nidn" x-model="selectedDosen && selectedDosen.nidn" required class="w-full bg-surface-container-lowest border border-outline-variant rounded-md px-3 py-2 text-on-surface focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-on-surface mb-1">Nama Lengkap</label>
                            <input type="text" name="nama" x-model="selectedDosen && selectedDosen.nama" required class="w-full bg-surface-container-lowest border border-outline-variant rounded-md px-3 py-2 text-on-surface focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-on-surface mb-1">Tipe Dosen</label>
                            <select name="tipe_dosen" x-model="selectedDosen && selectedDosen.tipe_dosen" required class="w-full bg-surface-container-lowest border border-outline-variant rounded-md px-3 py-2 text-on-surface focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all">
                                <option value="TETAP">TETAP</option>
                                <option value="LUAR_BIASA">LUAR BIASA</option>
                            </select>
                        </div>
                        <div class="flex items-center gap-2 mt-2">
                            <input type="checkbox" name="is_active" id="is_active_edit" value="1" :checked="selectedDosen?.is_active" class="w-4 h-4 rounded border-outline-variant text-secondary focus:ring-secondary">
        ...
        <div x-show="showDeleteModal" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4" style="display: none;">
            <div @click.away="showDeleteModal = false" x-transition class="bg-surface-container-lowest rounded-xl shadow-xl w-full max-w-sm overflow-hidden">
                <div class="p-6 text-center">
                    ...
                    <h3 class="font-h3 text-h3 text-on-surface mb-2">Hapus Dosen?</h3>
                    <p class="text-sm text-on-surface-variant mb-6">Apakah Anda yakin ingin menghapus data <strong x-text="selectedDosen?.nama"></strong>? Tindakan ini tidak dapat dibatalkan.</p>
                    
                    <form :action="`/dosen/${selectedDosen?.id}`" method="POST" class="flex gap-2 justify-center">
                        @csrf
                        @method('DELETE')
